    ...
    <v-card flat>
        ...
        >
                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        ...
                        <v-card>
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-form v-model="valid" ref="musicFileForm">
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
                                                <v-file-input :rules="[rules.required]" accept="audio/*"
                                                              @change="musicFileChange" label="请选择音乐文件"></v-file-input>
                                            </v-col>
                                        </v-row>
                                    </v-form>
                                    <v-form v-model="valid" ref="imageFileForm">
                                        <v-row align="center">
                                            <v-col class="pa-0" cols="10" sm="10" md="10">
                                                <v-file-input @change="fileChange" accept="image/*"
                                                              label="请选择音乐封面文件"></v-file-input>
                                            </v-col>
                                            <v-col cols="2">
                                                <v-img aspect-ratio="1" max-width="80" max-height="80"
                                                       :src="coverUrl"></v-img>
                                            </v-col>
                                        </v-row>
                                    </v-form>
                                    <v-form v-model="valid" ref="typeForm">
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
                                                <v-text-field :rules="[rules.required,rules.counter]"
                                                              v-model="editedItem.musicName" label="歌曲名"></v-text-field>
                                            </v-col>
                                        </v-row>
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
                                                <v-text-field v-model="editedItem.musicAuthor"
                                                              label="歌手"></v-text-field>
                                            </v-col>
                                        </v-row>
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
                                                <v-text-field v-model="editedItem.musicAlbum"
                                                              label="专辑"></v-text-field>
                                            </v-col>
                                        </v-row>
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
                                                <v-text-field v-model="editedItem.musicPublisDate"
                                                              label="发行时间"></v-text-field>
                                            </v-col>
                                        </v-row>
                                        <v-row>
                                            <v-col class="pa-0" cols="12" sm="12" md="12">
            ...
            <template v-slot:item.musicPublisDate="{ item }">
                <v-card color="rgba(0,0,0,0)" flat>{{formatDate(new Date(item.musicPublisDate))}}</v-card>
            </template>
            ...
            <template v-slot:item.typeStatusBoolean="{ item }">
                <v-switch inset v-model="item.typeStatusBoolean" @click.stop="changeStatus(item)"
                          class="mx-2"></v-switch>
            </template>
...
<script>
    ...
    export default {
        data: () => ({
            ...
            headers: [
                {text: "id", value: "id"},
                {text: "音乐id", width: 200, value: "musicId", sortable: false},
            ...
            desserts: [],
            editedIndex: -1,
            editedItem: {musicName: "", typeName: "流行"},
            defaultItem: {},
            valid: true,
            typenameRules: [
                v => !!v || '类型名不能为空！',
        ...
        methods: {
            ...
            musicFileChange(e) {
                ...
                var that = this;
                reader.onload = function (e) {
                    that.musicUrl = e.currentTarget.result
                }
                this.editedItem.musicName = e.name.split(".")[0];
